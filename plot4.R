...
## configure the plotting are to 2 rows and 2 columns
par(mfrow=c(2,2))

# plot the Global Active power plot
plot(d2$DateTime ,d2$Global_active_power ,type="l", xaxt="n", xlab="", ylab="Global Active Power")
# format x-label to dayname
axis.POSIXct(1, d2$DateTime, format="%A")

# plot the in 2nd quadrant
plot(d2$DateTime ,d2$Voltage ,type="l", xaxt="n", xlab="datetime", ylab="Voltage")
# format x-label to dayname
axis.POSIXct(1, d2$DateTime, format="%A")

# Plot submetering in 3rd quadrant
# plot submettring_1 first, then add submetering2 and 3.
...
# Plot reactive power in 4th quadrant
plot(d2$DateTime ,d2$Global_reactive_power ,type="l", xaxt="n", xlab="datetime", ylab="Global Reactive Power")
# format x-label to dayname
axis.POSIXct(1, d2$DateTime, format="%A")
dev.off()